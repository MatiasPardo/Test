...
public class PlanCompras extends Transaccion implements IImportadorItemCSV{
	...
	private Collection<ItemPlanCompras> items;
	
	@Override
	public String descripcionTipoTransaccion() {
		return "Plan de Compras";
	}

	public Collection<ItemPlanCompras> getItems() {
		return items;
	}

	public void setItems(Collection<ItemPlanCompras> items) {
		this.items = items;
	}
	...
	public ItemTransaccion crearItemDesdeCSV(String[] values) {
		...
		if (values.length >= cantidadCampos){
			...
			if (this.itemsPorProductoCSV.containsKey(codigoProducto)){
				...
				((ItemPlanCompras)item).setPlan(this);
				item.setProducto(producto);
				this.itemsPorProductoCSV.put(codigoProducto, item);
				this.getItems().add((ItemPlanCompras)item);
			}
