...
public class PlanProduccion extends Transaccion implements IImportadorItemCSV{
	...
	private Collection<ItemPlanProduccion> items;
	
	@Override
	public String descripcionTipoTransaccion() {
		return "Plan de Produccion";
	}

	public Collection<ItemPlanProduccion> getItems() {
		return items;
	}

	public void setItems(Collection<ItemPlanProduccion> items) {
		this.items = items;
	}
	...
	public ItemTransaccion crearItemDesdeCSV(String[] values) {
		...
		if (values.length >= cantidadCampos){
			...
			if (this.itemsPorProductoCSV.containsKey(codigoProducto)){
				...
				((ItemPlanProduccion)item).setPlan(this);
				item.setProducto(producto);
				this.itemsPorProductoCSV.put(codigoProducto, item);
				this.getItems().add((ItemPlanProduccion)item);
			}
